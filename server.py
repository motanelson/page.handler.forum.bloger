...
def load_posts(category, page, per_page=5):
    ...
    with get_db() as db:
        c = db.cursor()
        c.execute(
            "SELECT url, message FROM posts WHERE category=? ORDER BY id DESC LIMIT ? OFFSET ?",
...
def category_page(category):
    ...
    if page > 1:
        html += f'<a href="/{category}?page={page-1}">⬅ Anterior</a> '

    html += f" Página {page} de {total_pages} "

    if page < total_pages:
        html += f' <a href="/{category}?page={page+1}">Próxima ➡</a>'
    ...
    for p in range(1, total_pages + 1):
        if p == page:
            ...
            html += f'<a href="/{category}?page={p}">{p}</a> '
